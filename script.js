...
let customColors = JSON.parse(localStorage.getItem("pixelworld_custom_colors") || "[]")
...
const customColorEl=document.getElementById("customColor"), addCustomColorBtn=document.getElementById("addCustomColor");
const codeInput=document.getElementById("codeInput"), redeemCodeBtn=document.getElementById("redeemCodeBtn"), codeStatus=document.getElementById("codeStatus");
...
function allColors(){return [...COLORS,...customColors.filter(c=>!COLORS.includes(c))];}
...
function createPalette(container){
  ...
  allColors().forEach(color=>{
    const b=document.createElement("button"); b.type="button"; b.className="color"; b.style.background=color; b.dataset.color=color; b.title=color; b.setAttribute("aria-label","Couleur "+color);
    if(color===selectedColor)b.classList.add("selected");
    b.addEventListener("click",()=>selectColor(color)); container.appendChild(b);
  });
}
function refreshPalettes(){createPalette(palette);createPalette(paletteSide);}
refreshPalettes();
addCustomColorBtn?.addEventListener("click",()=>{
  const color=customColorEl.value.toLowerCase();
  if(!customColors.includes(color)){customColors.push(color);localStorage.setItem("pixelworld_custom_colors",JSON.stringify(customColors));}
  selectColor(color);refreshPalettes();showMessage("🎨 Couleur personnalisée ajoutée !");
});
...
function draw(){
  ...
  // Only the real map border, so the 2500×2500 world stays visible when zoomed out.
  ctx.strokeStyle="#3b3f50";ctx.lineWidth=Math.max(1,4/s);ctx.strokeRect(offsetX,offsetY,SIZE*s,SIZE*s);
}
...
async function placePixel(x,y){
  ...
  if(existing && existing.discord_id !== player.discord_id){showMessage("🚫 Ce pixel appartient à un autre joueur.");return;}
  try{
    ...
    pixels.set(x+","+y,{x,y,color:selectedColor,username:player.username,discord_id:player.discord_id,updated_at:new Date().toISOString()});
    ...
    showMessage(data.overwritten_own?"♻️ Ton pixel a été modifié.":data.critical?"✨ Pixel critique ! Aucun pixel consommé.":`Pixel posé en ${x}, ${y} !`); beep(data.critical?880:520,0.09);
  }catch{showMessage("Impossible de contacter le serveur.");}
}
function showPixelInfo(cell){const p=pixels.get(cell.x+","+cell.y);if(p){const date=new Date(p.updated_at);const owner=p.discord_id===player?.discord_id?"Toi":"Autre joueur";pixelInfo.innerHTML=`<b>Pixel ${cell.x}, ${cell.y}</b><br>🎨 ${escapeHtml(p.color)}<br>👤 ${escapeHtml(p.username)} (${owner})<br>🕒 ${date.toLocaleString("fr-FR")}`;}else pixelInfo.textContent=`Pixel ${cell.x}, ${cell.y} • Vide`;}
...
async function redeemCode(){
  if(!player){showMessage("Connecte-toi avec Discord pour utiliser un code.");return;}
  const code=(codeInput?.value||"").trim().toUpperCase(); if(!code)return;
  redeemCodeBtn.disabled=true;
  try{
    const res=await fetch(API_URL+"/api/codes/redeem",{method:"POST",headers:authHeaders({"Content-Type":"application/json"}),body:JSON.stringify({code})});
    const data=await res.json();
    if(!res.ok){codeStatus.textContent="❌ "+(data.error||"Code invalide.");return;}
    player=data.player;balanceEl.textContent=player.pixel_balance;maxEl.textContent=player.max_storage;totalPlacedEl.textContent=player.total_placed;codeStatus.textContent=`🎁 +${data.pixels_received} pixels récupérés !`;codeInput.value="";
  }catch{codeStatus.textContent="❌ Serveur inaccessible.";}finally{redeemCodeBtn.disabled=false;}
}
